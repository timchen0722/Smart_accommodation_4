...
**Architecture:** 新增兩個模組做責任切分 —— `modules/platform_analytics.py` 是**純 pandas 計算層**（不 import streamlit，可用 pytest 直接測），`modules/platform_sections.py` 是**渲染層**（呼叫計算層＋既有 UI 元件）。頁面檔只負責側欄、分頁骨架與呼叫 render 函式。既有模組（`portfolio_sections`、`absa_sections`、`backend_v2_sections`、`notify_center`、`vacancy_model`）全部重用，不重寫。
