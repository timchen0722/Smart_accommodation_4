...
### 2026-07-24 討論用分析：建議作廢（待使用者確認）

本計畫的目標是「**移除重複的象限統計/表格**，改成可點選的分類卡片」。
重新檢視後，**那個重複問題已經在 2026-07-22～23 的房東入口改版中被解掉了**，
只是走了另一條路徑：

| 本計畫的目標 | 現況（實測） |
|---|---|
| 移除重複的象限統計/表格 | **已達成** —— `AI_README` 進度記錄「已改為單一表格呈現，移除重複的象限統計圖卡」；`pages/1` 目前只有一個 `quadrant_summary_table()`（`:207`），沒有第二份統計 |
| 可點選的分類卡片（篩選房源） | **已有等效互動** —— `pages/1:276` 的 `st.radio("營運狀態")` 提供同樣的分類篩選，且卡片標題會顯示篩選後間數 |
| 分類專屬的房源明細 | **已達成** —— 篩選後的房源卡片即為該分類的明細 |
| 統一分類文案為單一來源 | **已達成** —— `quadrant.QUADRANTS` 的第五類已於階段 3 改吃 `design_tokens.STATUS_NO_DATA` |
| 新增 `quadrant_sections.py` 與 4 個 API | 未做，且**現在已無必要**（沒有第二個呼叫端需要共用） |

唯一沒被涵蓋的是計畫裡的 `validate_partition()`（斷言各分類房源數加總 == 去重總數）。
那是防禦性檢查，目前沒有已知的重複計數症狀。

**建議:標記為作廢（superseded），理由如上。** 若日後象限分類要出現在第二個頁面
（例如後台也要看象限），再回頭抽 `quadrant_sections.py` 不遲。

在使用者確認前，本計畫維持「未實作」狀態，不要開工。
